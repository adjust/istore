...
ROLLBACK;
BEGIN;
-- extend istore should correctly aggregate using sum_floor;
-- ./spec/istore/aggregates_spec.rb:49;
CREATE EXTENSION istore;
CREATE TABLE test (v istore);
INSERT INTO test SELECT istore(array_agg(x), array_agg(x)) FROM generate_series(1, 21, 2) AS a(x);
INSERT INTO test SELECT istore(array_agg(x), array_agg(x)) FROM generate_series(2, 22, 2) AS a(x);
INSERT INTO test SELECT istore(array_agg(x), array_agg(x)) FROM generate_series(1, 21, 2) AS a(x);
SELECT SUM_FLOOR(v, 10) FROM test;
ROLLBACK;
BEGIN;
-- extend bigistore should correctly aggregate using sum_floor;
-- ./spec/istore/aggregates_spec.rb:49;
CREATE EXTENSION istore;
CREATE TABLE test (v bigistore);
INSERT INTO test SELECT bigistore(array_agg(x), array_agg(x)) FROM generate_series(1, 21, 2) AS a(x);
INSERT INTO test SELECT bigistore(array_agg(x), array_agg(x)) FROM generate_series(2, 22, 2) AS a(x);
INSERT INTO test SELECT bigistore(array_agg(x), array_agg(x)) FROM generate_series(1, 21, 2) AS a(x);
SELECT SUM_FLOOR(v, 10) FROM test;
ROLLBACK;
